            ...
            if (eline[j].ifUprade == 0) {//没有被升格或者不为空，进行下面操作
                for (int k = 0; k < o; k++) {//从消元子1靠前的开始消元，所以为倒序
                    if (eline[j].bit[(i - k) / 8] & (1 << (i - k) % 8)) {//如果被消元行i-k位置为1，则可以用eliminer[i-k]消元
                        ...
                        //    __m128i vr = _mm_loadu_epi8(eliminer[i - k] + ss);
                        //    __m128i tmp = _mm_xor_si128(ve, vr);
                        //    _mm_storeu_epi8(eline[j].bit + ss, tmp);

                        //}
                        //for (ss; ss <= (i - k) / 8; ss++)  //第(i-k)/8个可能对不需要操作的位进行了异或，但不影响结果
                        //    eline[j].bit[ss] ^= eliminer[i - k][ss];

                        //int ss = 0;
                        //for (ss; ss + 16 <= (i - k) / 8; ss += 16) { //32个字节，8个int
                        //    uint8x16_t ve = vld1_u8((const uint8_t*)&eline[j].bit[ss]);
                        //    uint8x16_t vr = vld1_u8(( const uint8_t*)&eliminer[i - k][ss]);
                        //
                        //    
                        //    uint8x16_t tmp=veorq_u8(ve,vr);
                        //    vst1_u8(( uint8_t*)&eline[j].bit[ss], tmp);
