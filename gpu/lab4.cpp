...
double gpu_kernel(float* A, float* B, float* C,
    ...
    float subA[8][tileY] = { 0.0f };
    float subB[8][tileX] = { 0.0f };
    ...
    for (int k = 0; k < N; k+=8) {
        ...
        // read data to register
            ...
            for (int m = 0; m < tileY; m++) {
                subA[0][m] = A[(row + m) * N + k];
                subA[1][m]= A[(row + m) * N + k+1];
                subA[2][m] = A[(row + m) * N + k+2];
                subA[3][m] = A[(row + m) * N + k + 3];
                subA[4][m] = A[(row + m) * N + k+4];
                subA[5][m] = A[(row + m) * N + k + 5];
                subA[6][m] = A[(row + m) * N + k + 6];
                subA[7][m] = A[(row + m) * N + k + 7];
            }

            for (int p = 0; p < tileX; p++) {
                subB[0][p] = B[k * N + p + col];
                subB[1][p] = B[(k+1)* N + p + col];
                subB[2][p] = B[(k+2)* N + p + col];
                subB[3][p] = B[(k+3) * N + p + col];
                subB[4][p] = B[(k+4)* N + p + col];
                subB[5][p] = B[(k + 5) * N + p + col];
                subB[6][p] = B[(k + 6) * N + p + col];
                subB[7][p] = B[(k + 7) * N + p + col];
            }
        
        
        for (int m = 0; m < tileY; m++) {
            for (int p = 0; p < tileX; p++) {
                sum[m][p] += subA[0][m] * subB[0][p];
                sum[m][p] += subA[1][m] * subB[1][p];
                sum[m][p] += subA[2][m] * subB[2][p];
                sum[m][p] += subA[3][m] * subB[3][p];
                sum[m][p] += subA[4][m] * subB[4][p];
                sum[m][p] += subA[5][m] * subB[5][p];
                sum[m][p] += subA[6][m] * subB[6][p];
                sum[m][p] += subA[7][m] * subB[7][p];
            }
        }
